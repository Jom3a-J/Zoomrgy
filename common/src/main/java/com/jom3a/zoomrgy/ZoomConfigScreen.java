...
/**
 * Builds the Cloth Config screen. Shared, because Cloth ships for both loaders with the same API -
 * only the hook that opens it differs, so each loader supplies that and calls in here.
 */
public final class ZoomConfigScreen {

    private ZoomConfigScreen() {
    }

    public static Screen create(Screen parent) {
        // Reads the stored preview picture, taking one from the game the first time.
